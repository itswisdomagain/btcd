...
func (s *server) pushInventory(sp *serverPeer, iv *wire.InvVect,
	...
	case wire.InvTypeMix:
		return s.pushMixMsg(sp, &iv.Hash, doneChan)
...
func (s *server) pushMixMsg(sp *serverPeer, hash *chainhash.Hash, doneChan chan<- struct{}) error {
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	s.p2pDowngrader = peer.NewP2PDowngrader(uint(s.targetOutbound) + 1)
